...
use Exception;
use PixelgradeLT\PartTemplate\Utils\JSONCleaner;
use function PixelgradeLT\PartTemplate\doing_it_wrong;
use const PixelgradeLT\PartTemplate\LOG_DIR;
...
class FileLogHandler extends LogHandler {
	...
	public function __construct( int $log_size_limit = null ) {
		...
		$this->log_size_limit = apply_filters( 'pixelgradelt_part_template/log_file_size_limit', $log_size_limit );
	...
	public function clear( string $handle ): bool {
		...
		do_action( 'pixelgradelt_part_template/log_clear', $handle );
	...
	public function remove( string $handle ): bool {
		...
		if ( isset( $logs[ $handle ] ) && $logs[ $handle ] ) {
			...
			do_action( 'pixelgradelt_part_template/log_remove', $handle, $removed );
		}
		return $removed;
	}
