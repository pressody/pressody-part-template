...
use function Pressody\PartTemplate\doing_it_wrong;
use function Pressody\PartTemplate\is_running_unit_tests;
...
final class Logger extends BaseIO {
	...
	public function __construct( string $minimum_level, array $handlers = null ) {
		...
		if ( ! empty( $handlers ) && is_array( $handlers ) ) {
			foreach ( $handlers as $handler ) {
				...
				if ( is_object( $handler ) && is_array( $implements ) && in_array( 'Pressody\PartTemplate\Logging\Handler\LogHandlerInterface', $implements, true ) ) {
					...
					doing_it_wrong(
						...
						/* translators: 1: class name 2: WC_Log_Handler_Interface */
							...
							'<code>Pressody\PartTemplate\Logging\Handler\LogHandlerInterface</code>'
						),
						'0.1.0'
					);
				}
	...
	public function log( $level, $message, array $context = [] ) {
		if ( ! LogLevels::is_valid_level( $level ) ) {
			...
			doing_it_wrong( __METHOD__, sprintf( __( '%1$s was called with an invalid level "%2$s".', 'pressody_retailer' ), '<code>Pressody\PartTemplate\Logging\Logger::log</code>', $level ), '0.1.0' );
		}
